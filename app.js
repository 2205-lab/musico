  ...
  try {
    const response = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are Wavmind, an expert AI assistant for music producers. Format using Slack mrkdwn. Use *text* for bold. Use • for bullets. Never use ** or # headers. Keep responses clean and scannable.',
        },
...
app.message(async ({ message, say }) => {
  ...
  const keywords = ['muddy','808','sidechain','compress','reverb','delay','mixing','mastering','plugin','vst','fl studio','ableton','logic pro','pro tools','melody','chord','bass line','hi-hat','kick drum','snare','bpm'];
    ...
    try {
      ...
      if (response) {
        await say({
          ...
          blocks: [
            section(`🎛️ *Wavmind:* ${response}`),
            context('Type `/wavmind` for all features'),
          ],
        });
      }
  ...
  try {
    ...
    if (compareSession) {
      if (compareSession.status === 'waiting_your_track') {
        await client.chat.postMessage({
          channel: channelId,
          blocks: [header('🎵 Scanning Your Track...'), section(`*File:* ${file.name}`), context('⏳ Step 1 of 2')],
        });
        ...
        await client.chat.postMessage({
          ...
          blocks: [
            ...
            context('⏳ Step 2 of 2 · Wavmind compares both automatically'),
          ],
        });
        ...
        await client.chat.postMessage({
          channel: channelId,
          blocks: [header('🔍 Scanning Reference...'), section(`*File:* ${file.name}`), context('⏳ Generating report...')],
        });
        const analysis = await analyzeAudioFile(file.url_private_download, file.name);
        if (!analysis || analysis.error) {
          await client.chat.postMessage({ channel: channelId, blocks: [header('❗ Scan Failed'), section('Try MP3 under 10MB.')] });
...
Give specific EQ, compression, bass advice to close each gap. Top 3 most important changes. Real plugin names.`);
    ...
    await client.chat.postMessage({
      channel: channelId,
      blocks: [header('🎵 Scanning Your Track...'), section(`*File:* ${file.name}`), context('⏳ Analyzing...')],
    });
    const analysis = await analyzeAudioFile(file.url_private_download, file.name);
    if (!analysis || analysis.error) {
      await client.chat.postMessage({
...
function scheduleWeeklyDigest(client) {
    ...
    try {
      for (const userId of Object.keys(global.weeklyStats)) {
        ...
        await client.chat.postMessage({
          channel: userId,
          blocks: [
            header('📊 Weekly Production Report'),
  ...
  setTimeout(() => {
    sendDigest();
    setInterval(sendDigest, 7 * 24 * 60 * 60 * 1000);
  }, nextMonday - now);
...
app.command('/wavmind', async ({ command, ack, respond, client }) => {
  ...
  if (lower.startsWith('mix compare') || lower.startsWith('mix-compare')) {
    ...
    await respond({
      ...
      blocks: [
        ...
        section('*Step 3* — Wavmind compares both and gives you:\n• Side-by-side energy, bass, brightness\n• ✅ Match or ⚠️ Gap indicators\n• Specific AI advice to close each gap'),
        divider(),
  ...
  if (lower.startsWith('daw')) {
    ...
    for (const daw of dawList) {
      for (const kw of daw.keywords) {
        if (dawInput.toLowerCase().startsWith(kw)) {
          detectedDAW = daw.name;
          question = dawInput.slice(kw.length).trim();
          break;
        }
      }
    ...
    const [tav, ai] = await Promise.all([
      ...
      askAI(`Expert ${detectedDAW} instructor. Answer: "${question}". Numbered steps with bold key terms.`),
    ]);
  ...
  if (lower.startsWith('samples')) {
    ...
    if (!query) {
      await respond({ blocks: [header('🎵 Free Samples'), section('`/wavmind samples drums`\n`/wavmind samples piano`\n`/wavmind samples bass`\n`/wavmind samples synth`\n`/wavmind samples ambient`'), context('Creative Commons — free to use')] });
      return;
    }
  ...
  if (lower.startsWith('collab')) {
    ...
    if (subL.startsWith('start')) {
      ...
      if (getCollabSession(command.channel_id)) { await respond({ blocks: [header('⚠️ Active Session'), section('`/wavmind collab end` first')] }); return; }
      startCollabSession(command.channel_id, name, userId);
      await respond({
        response_type: 'in_channel',
        blocks: [
          header('🤝 Collab Started'),
          section(`*Track:* "${name}"\n*By:* <@${userId}>`),
          divider(),
          twoCol('💡 `/wavmind collab idea [idea]`', '🎚️ `/wavmind collab feedback [fb]`'),
          twoCol('✅ `/wavmind collab decision [dec]`', '📋 `/wavmind collab summary`'),
    ...
    if (subL.startsWith('status')) {
      ...
      await respond({ blocks: [header('📊 Status'), section(`*"${s.trackName}"* by <@${s.startedBy}>`), divider(), twoCol(`💡 ${s.ideas.length} ideas`, `🎚️ ${s.feedback.length} feedback`), twoCol(`✅ ${s.decisions.length} decisions`, `⏱️ ${new Date(s.startedAt).toLocaleTimeString()}`), context('`/wavmind collab summary` · `/wavmind collab end`')] });
      return;
    }
    ...
    if (subL.startsWith('end')) {
      const s = getCollabSession(command.channel_id);
      if (!s) { await respond({ blocks: [header('❗ No Session')] }); return; }
      const r = await askAI(`Final report for "${s.trackName}": IDEAS: ${s.ideas.map(i=>i.text).join(', ')||'None'} FEEDBACK: ${s.feedback.map(f=>f.text).join(', ')||'None'} DECISIONS: ${s.decisions.map(d=>d.text).join(', ')||'None'}. Overview, decisions, action items.`);
      endCollabSession(command.channel_id);
      await respond({ response_type: 'in_channel', blocks: [header('🏁 Session Complete'), section(`*"${s.trackName}"*`), divider(), section(r || 'Error'), context('`/wavmind collab start "Name"` for new session')] });
      return;
    }
  ...
  if (lower.startsWith('compare')) {
    ...
    await respond({
      blocks: [
        ...
        divider(),
        section(ai || 'Error'),
        context('`/wavmind reference [track]` for specific song'),
      ],
    });
    return;
  }
    ...
    if (f) {
      ...
      await respond({
        blocks: [
          ...
          divider(),
          section(r || 'Error'),
          context('`/wavmind mix compare start` to compare your audio'),
        ],
      });
    } else {
  ...
  if (lower.startsWith('feedback')) {
    ...
    const r = await askAI(`Professional mixing feedback for: "${desc}". EQ, compression, stereo width, frequency balance. Clear sections with emojis.`);
  ...
  if (lower.startsWith('mixfeedback')) {
    ...
    await respond({
      blocks: [
        ...
        divider(),
        section(r || 'Error'),
        context('`/wavmind mix compare start` to compare audio files'),
      ],
    });
    return;
  }
